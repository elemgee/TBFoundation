...
package org.treasureboat.foundation.formatters;
 ...
 */
public class TBFSimpleHTMLFormatter extends java.text.Format {
  /**
   ...
   */
  static final Logger log = LoggerFactory.getLogger(TBFSimpleHTMLFormatter.class);

  /** holds the reference to the shared formatter */
  private static TBFSimpleHTMLFormatter _formatter;
   ...
   */
  public static TBFSimpleHTMLFormatter formatter() {
    if (_formatter == null) {
      _formatter = new TBFSimpleHTMLFormatter();
    }
    return _formatter;
  }
   ...
   *        to be formatted
   * @param buffer
   *        to have the formatted object appended to
   * @param fp
   *        ignored parameter
   *        
   * @return buffer 
   *        after having the format appended to it.
   */
  @Override
  ...
  public String applyFormat(Object anObject) throws IllegalArgumentException {
    ...
    newString = StringUtils.replace((String)anObject, TBFConstants.TAB, HTMLTab());

    // Convert new-lines in the argument (which must be a String) to HTML breaks.
    return StringUtils.replace(newString, TBFConstants.LF, TBFConstants.HTML_Return);
  }
  ...
  public Object parseObject(String inString) throws java.text.ParseException {
    ...
    // Convert new-lines in the argument (which must be a String) to HTML breaks.
    newString = StringUtils.replace(inString, TBFConstants.HTML_Return, TBFConstants.LF);

    // Convert tabs in the argument (which must be a String) to HTML spacers.
    return StringUtils.replace(newString, HTMLTab(), TBFConstants.TAB);
  }
