...
public class TBFCompressionUtilities {
  /** 
   * <a href="http://wiki.wocommunity.org/display/documentation/Wonder+Logging">new org.slf4j.Logger</a> 
   */
  static final Logger log = LoggerFactory.getLogger(TBFCompressionUtilities.class);
  ...
  public static NSData gzipInputStreamAsNSData(InputStream input, int length) {
    try {
      TBFRefByteArrayOutputStream bos = new TBFRefByteArrayOutputStream(length);
      if (input != null) {
        GZIPOutputStream out = new GZIPOutputStream(bos);
        try {
          TBFFileUtilities.writeInputStreamToOutputStream(input, true, out, false);
        }
        finally {
          try {
            out.finish();
  ...
  public static NSData gzipNSDataAsNSData(NSData data) {
    ...
    if (data != null) {
      gzippedData = TBFCompressionUtilities.gzipByteArrayAsNSData(data._bytesNoCopy(), 0, data.length());
    }
    return gzippedData;
  }
  ...
  public static byte[] gunzipByteArray(byte[] input) {
    try {
      ...
      byte[] uncompressedData = TBFFileUtilities.bytesFromInputStream(in);
  ...
  public static byte[] zipByteArray(byte[] input) {
    return TBFCompressionUtilities.zipByteArray(input, "tmp");
  }
